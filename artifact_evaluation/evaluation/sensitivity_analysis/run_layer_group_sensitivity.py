...
MODEL_CONFIG = {
    "bert": {
        ...
        "layer_groups_sweep": [2, 3, 4, 6, 8, 12, 24],
        "cfreqs": [0, 10, 25, 50],
        "iters": 350,
        "batch_size": 3,
        "max_async": 4,
...
def _log_path(model, K, cf):
    return os.path.join(OUTPUT_DIR, model, f"log_{model}_K{K}_cf{cf}.txt")
...
def run_experiments(models):
    for model in models:
        ...
        for K in cfg["layer_groups_sweep"]:
            for cf in cfg["cfreqs"]:
                log = _log_path(model, K, cf)
                if os.path.exists(log):
                    print(f"[SKIP] {log} exists")
                    continue
                print(f"[RUN] {model}  K={K}  cfreq={cf}")
...
def plot_sensitivity(all_results):
    ...
    for idx, (model, df) in enumerate(all_results.items()):
        ...
        for cf in plot_cfreqs:
            ...
            if popt is not None:
                ...
                ax.plot(K_fine, thr_fine_norm, '--',
                        color=colors_cf.get(cf, 'gray'), alpha=0.5, linewidth=1.5)

                K_opt_clamp = max(K_vals.min(), min(K_opt, K_vals.max()))
                thr_opt = throughput_model(K_opt_clamp, *popt)
                if base_thr and base_thr > 0:
                    thr_opt_norm = thr_opt / base_thr
                else:
                    thr_opt_norm = thr_opt
                ax.axvline(x=K_opt_clamp, color=colors_cf.get(cf, 'gray'),
                           linestyle=':', alpha=0.6, linewidth=1)

        ax.set_xlabel('Number of layer groups ($K$)', fontsize=14)
        if idx == 0:
        ...
        for cf in plot_cfreqs:
            ...
            if popt is not None:
                ...
                print(f"  {cfg['display_name']:16s}  interval={cf:3d}  "
                      ...
                      f"K*={K_opt:.1f}  chunk/stream={chunk_size_mb:.0f}MB")
    ...
    if fit_results:
        ...
        actual_K = {"BERT-Large": 6, "OPT-1.3B": 6, "Transformer-XL": 2}

        for r in fit_results:
            key = (r["Model"], r["Interval"])
            if key not in [(r2["Model"], r2["Interval"]) for r2 in models_plotted]:
                models_plotted.append(r)
                k_opt_vals.append(float(r["K*"]))
                k_actual_vals.append(actual_K.get(r["Model"], 0))

        x = np.arange(len(models_plotted))
        labels = [f"{r['Model']}\n(int={r['Interval']})" for r in models_plotted]
        width = 0.35

        ax.bar(x - width / 2, k_actual_vals, width, label='Actual $K$', color='#3498DB')
        ax.bar(x + width / 2, k_opt_vals, width, label='Optimal $K^*$', color='#E74C3C')

        ax.set_ylabel('Layer group count ($K$)', fontsize=14)
        ax.set_xticks(x)
