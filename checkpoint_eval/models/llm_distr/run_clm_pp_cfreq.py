...
class ModelArguments:
    ...
    bench_total_steps: int = field(
        default = 1000,
        metadata={
            "help": ("Total steps (for benchmarking)")
            }
    )
    path_to_pmem: str = field(
        default = '',
...
def main():
    ...
    if training_args.do_train:
        train_dataset = HuggingFaceDatasetWrapper(lm_datasets["train"], tokenizer)

    # CHECKPOINT-RELATED
    bench_total_steps = model_args.bench_total_steps
    cfreq = model_args.cfreq
    path_to_pmem = model_args.path_to_pmem
    ...
    model_engine, optimizer, train_loader, lr_schdlr = deepspeed.initialize(
        ...
        checkp_type='CheckFreq',
        checkp_params={
            'in_progress_snapshot': in_progress_snapshot
        }
    )
