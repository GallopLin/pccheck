...
class ModelArguments:
    ...
    pipeline_stages: int = field(
        default=2,
        metadata={
            "help": ("Number of pipeline stages for BLOOM conversion. Set to 1 to disable pipeline splitting.")
        }
    )
    disable_set_storage: bool = field(
        default=False,
        metadata={
...
def main():
    ...
    model_name_lower = (model_args.model_name_or_path or "").lower()
    if "bloom" in model_name_lower:
        ...
        raise ValueError(
            f"Unsupported model for pipeline conversion: {model_args.model_name_or_path}. "
            "Expected a BLOOM or OPT model name."
        )
