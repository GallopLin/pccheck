...
def main():
    ...
    parser.add_argument(
        "--num_layer_groups", default=6, type=int, help="Number of layer groups for multistream checkpoint"
    )
    parser.add_argument(
        "--c_lib_path",
        default="",
    ...
    if args.do_train:
        ...
        # else:
        # ✅ 使用标准 AdamW 而不是 BertAdam，因为 MultiStreamCheckpoint 需要标准的 exp_avg/exp_avg_sq 状态键
    ...
    if args.do_train:
        ...
        # ✅ 在训练循环之前初始化 multistream checkpoint 相关变量
        ms_checkpoint = None
        ms_optimizer = None
        param_layout = None
        gpu_ar = None
        ...
        for epoch in range(1):  # (int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_iter):
                ...
                # Note: fp16 disabled for multistream, use FP32 only
                loss.backward()
                ...
                # ✅ 与 CheckFreq/GPM/PCcheck 保持一致的 checkpoint 条件
                ...
                if should_enter_checkpoint_block:
                    if args.cfreq > 0:
                        ...
                        # 注意: gpu_ar, total_size, param_layout 已经在训练循环外初始化
        ...
        print(
            f"-- BENCHMARK ENDED: Total time: {time_to_train} sec, Number of iterations: {step}, Number of checkpoints: {checkpoints}"
        )
