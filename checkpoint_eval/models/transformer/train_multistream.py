...
# #############################################################################
# #region agent log (ndjson bootstrap)
# NOTE: Debug-mode runtime instrumentation. Do NOT remove until post-fix verified.
import json as _pcc_json
import time as _pcc_time
...
def _pccheck_log(hypothesisId: str, location: str, message: str, data: dict, runId: str = "pre-fix"):
    try:
        payload = {
            ...
            "message": message,
            "data": data,
            "timestamp": int(_pcc_time.time() * 1000),
        }
        with open(_PCCHECK_DEBUG_LOG_PATH, "a", encoding="utf-8") as f:
            f.write(_pcc_json.dumps(payload, ensure_ascii=False) + "\n")
    except Exception:
        pass

# Force local pccheck code to be used (avoid picking up an installed stale copy).
_PCCHECK_LOCAL_ROOT = "/home/linzhicheng/code/pccheck"
if _PCCHECK_LOCAL_ROOT not in sys.path:
    sys.path.insert(0, _PCCHECK_LOCAL_ROOT)
_pccheck_log(
    "H0",
    "train_multistream.py:bootstrap",
    "bootstrapped debug log + forced local pccheck path",
    {
        "pccheck_local_root": _PCCHECK_LOCAL_ROOT,
        "sys_path0": sys.path[0] if sys.path else None,
        "cwd": os.getcwd() if "os" in globals() else None,
...
# #############################################################################
# #region agent log (import resolution)
try:
    import checkpoint_eval.pccheck.multistream_checkpoint as _msc
    _pccheck_log(
        "H0",
        "train_multistream.py:imports",
        "resolved multistream_checkpoint module",
        {"multistream_checkpoint_file": getattr(_msc, "__file__", None)},
    )
except Exception as _e:
    _pccheck_log(
        "H0",
        "train_multistream.py:imports",
        "failed to resolve multistream_checkpoint module",
        {"error": str(_e)},
    )
# #endregion
# #############################################################################
...
def parse_args():
    ...
    general.add_argument("--cfreq", default=10, type=int, help="Checkpoint Frequency")
    general.add_argument(
        "--bench_total_steps", default=1000, type=int, help="Number of iterations"
    )
    general.add_argument(
        "--max_async",
        default=2,  # ✅ 增加到2，允许checkpoint流水线
        type=int,
    ...
    general.add_argument(
        "--num_layer_groups",
        default=4,  # Transformer-XL: 16层，每组4层（与BERT一致）
        type=int,
        help="Number of layer groups for multistream checkpoint",
    )
...
def train(
    ...
    if args.cfreq > 0:
        # Build param layout for multistream checkpoint
    ...
    for batch, (data, target, seq_len, _) in enumerate(
        ...
        if batch == args.bench_total_steps:
            break
        ...
        # For multistream checkpoint, no need to wait for GPU copy (handled internally)
        ...
        # ✅ 修复：与 CheckFreq/GPM/PCcheck 保持一致的 checkpoint 条件
        # 条件：(batch == warmup) 或者 ((cfreq > 0) 且 (达到 checkpoint 间隔))
        # 这确保在 warmup 结束时执行第一次 checkpoint，与其他方法一致
        should_enter_checkpoint_block = (batch == warmup) or (
            (args.cfreq > 0) and (steps_since_checkp == args.cfreq - 1)
        )
        
        if should_enter_checkpoint_block:
            # 只有 cfreq > 0 时才真正执行 checkpoint
            if args.cfreq > 0:
                ...
                if ms_checkpoint is None:
                    ...
                    ms_optimizer = ms_checkpoint.create_optimizer(optimizer, model)
                    print(f"MultiStreamCheckpoint initialized with {args.num_layer_groups} layer groups")
                
                print(f"Save checkpoint at step {batch}, train_step {train_step}")
                
                # ✅ 关键：用 step_with_callback 替代 optimizer.step()
                # step_with_callback 内部会执行 optimizer 更新并触发 checkpoint
            ...
            # ✅ warmup 结束时重置计时器（与其他方法一致）
            if batch == warmup:
                print(f"Start clock!")
                start_train_time = time.time()
        else:
            # 不需要 checkpoint：用普通 step
            if args.fp16 and args.amp == "pytorch":
                scaler.step(optimizer)
                scaler.update()
            else:
        ...
        # 注意：计时器重置已移动到 checkpoint 条件块中，与 CheckFreq/GPM/PCcheck 保持一致
        ...
        print(f"Batch {batch} took {time.time()-start_iter} sec")
        start_iter = time.time()

    # Shutdown multistream checkpoint
    if ms_checkpoint is not None:
        ms_checkpoint.shutdown()

    total_train_time = time.time() - start_train_time
    print(
        f"-- BENCHMARK ENDED: Total time: {total_train_time} sec, Number of iterations: {batch}, Number of checkpoints: {checkpoints}"
    )
...
def main():
    ...
    with TimeoutHandler() as timeout_handler:
        try:
            for epoch in range(1):  # itertools.count(start=start_epoch):
                if args.roll:
