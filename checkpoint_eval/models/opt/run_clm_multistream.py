...
class ModelArguments:
    ...
    max_async: int = field(
        default = 3,  # OPT-1.3B: 增加并行checkpoint数以支持大模型
        metadata={
    ...
    num_layer_groups: int = field(
        default = 12,  # OPT-1.3B: 24层，每组2层，控制每组数据量≈1.7GB
        metadata={
    ...
    psize: int = field(
        default = 1,
        metadata={
            "help": ("Bucket size ratio will be total_size/psize")
            }
    )
    dram_ratio: float = field(
        default = 2.0,
        metadata={
            "help": ("DRAM buffer size will be dram_ratio*total_size")
            }
    )
    ...
    multistream_checkpoint_file: str = field(
        default = "multistream.chk",
        metadata={"help": "Checkpoint file name for multistream metadata"}
    )
    multistream_metadata_file: str = field(
        default = "multistream.chk.metadata.json",
        metadata={"help": "Metadata file name for multistream checkpoint"}
    )
...
def main():
    # ✅ 强制禁用分布式训练（单机单卡模式）
    # 必须在 TrainingArguments 实例化之前清除环境变量，
    # 因为 PartialState 会在 _setup_devices 中读取这些环境变量
    import os
    os.environ.pop("LOCAL_RANK", None)
    os.environ.pop("RANK", None)
    os.environ.pop("WORLD_SIZE", None)
    ...
    trainer = MultistreamTrainer(
        ...
        c_lib_path = model_args.c_lib_path,
    ...
    if training_args.do_train:
        ...
        if training_args.output_dir is not None:
            trainer.export_multistream_metadata(
                output_dir=training_args.output_dir,
                checkpoint_file=model_args.multistream_checkpoint_file,
                metadata_file=model_args.multistream_metadata_file,
