...
class MultistreamTrainer:
    ...
    def __init__(
        ...
        cfreq=10,
        c_lib_path="",
        bench_total_steps=1000,
        max_async=2,  # ✅ 增加到2，允许checkpoint流水线，避免等待上一个msync完成
        num_threads=1,
        num_layer_groups=6,
        psize=1,
        dram_ratio=2.0
        ...
        self.bench_total_steps = bench_total_steps
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            optimizer_cls, optimizer_kwargs = MultistreamTrainer.get_optimizer_cls_and_kwargs(
                self.args
            )
    ...
    def export_multistream_metadata(
        self,
        output_dir: str,
        checkpoint_file: str = "multistream_checkpoint.chk",
        metadata_file: Optional[str] = None,
        optimizer: Optional[torch.optim.Optimizer] = None,
        chunk_groups: Optional[List[List[int]]] = None,
    ) -> Optional[str]:
        """
        导出 multistream 元数据（复用 MultiStreamCheckpoint.export_metadata）。

        Args:
            output_dir: 输出目录
            checkpoint_file: 多流 checkpoint 文件名（用于写入 metadata 中）
            metadata_file: 元数据文件名，默认使用 checkpoint_file + .metadata.json
            optimizer: 可选，导出 optimizer param_groups/steps
            chunk_groups: 可选，自定义 layer group 划分
        """
        if metadata_file is None:
            metadata_file = f"{checkpoint_file}.metadata.json"

        os.makedirs(output_dir, exist_ok=True)
        metadata_path = os.path.join(output_dir, metadata_file)
        optimizer = optimizer if optimizer is not None else self.optimizer

        if self._ms_checkpoint is None:
            logger.warning(
                "Multistream metadata export skipped: MultiStreamCheckpoint is not initialized."
            )
            return None

        self._ms_checkpoint.filename = checkpoint_file
        self._ms_checkpoint.metadata_path = metadata_path
        return self._ms_checkpoint.export_metadata(
            metadata_path, optimizer=optimizer, chunk_groups=chunk_groups
        )
    ...
    def _wrap_model(self, model, training=True, dataloader=None):
        ...
        # Distributed training using PyTorch FSDP
            ...
            if self.args.fsdp_config["fsdp_min_num_params"] > 0:
                ...
                self.args.fsdp_config.get("fsdp_transformer_layer_cls_to_wrap", None)
                is not None
            ):
    ...
    def _inner_training_loop(
        ...
        # Multistream checkpoint 相关变量
        ms_checkpoint = self._ms_checkpoint
        ms_optimizer = None
        gpu_ar = None
        total_size = 0
        param_layout = None
        ...
        if self.cfreq > 0:
            param_layout = build_param_layout(model, self.optimizer)
        ...
        checkpoints = 0
        # 统一计时口径：只在 warmup 结束后开始计时一次。
        # OPT-2.7B 需要 ~50 步让 set_storage 的瞬态开销收敛。
        start_training = None
        warmup = 50
        started_timing = False

        for epoch in range(1):  # range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                if step == self.bench_total_steps:
                    break
                ...
                if total_batched_samples % args.gradient_accumulation_steps == 0 or (
                    ...
                    # Multistream: no need to wait for GPU copy (handled internally)

                    # Reduce stdout overhead during throughput benchmarks.
                    # Enable with: PCCHECK_VERBOSE=1
                    if os.environ.get("PCCHECK_VERBOSE", "0") == "1":
                        print(
                            f"MULTISTREAM, cfreq={self.cfreq}, steps_since_checkp={steps_since_checkp}, num_layer_groups={self.num_layer_groups}"
                        )
                    
                    # 检查是否需要进入 checkpoint 逻辑
                    should_enter_checkpoint_block = (step == warmup) or (
                        (self.cfreq > 0) and steps_since_checkp == self.cfreq - 1
                    )
                    
                    if should_enter_checkpoint_block:
                        if self.cfreq > 0:
                            # 初始化 MultiStreamCheckpoint（如果还没初始化）
                            if ms_checkpoint is None:
                                if os.environ.get("PCCHECK_VERBOSE", "0") == "1":
                                    print(f"Initializing MultiStreamCheckpoint...")
                                ...
                                if os.environ.get("PCCHECK_VERBOSE", "0") == "1":
                                    print(
                                        f"MultiStreamCheckpoint initialized with {self.num_layer_groups} layer groups"
                                    )
                            ...
                            if self.do_grad_scaling:
                                ...
                                ms_optimizer.step_with_checkpoint(wait=False)
                            
                            steps_since_checkp = 0
                            checkpoints += 1
                        else:
                            # cfreq == 0: 不做 checkpoint，执行普通 step
                        ...
                        # 注意：不再在 warmup checkpoint 或第一个 checkpoint 处重置计时。
                        # 计时在 warmup 结束后统一开始。
                    else:
                        # 不需要 checkpoint：执行普通 step
                        optimizer_was_run = True
                        if is_torch_tpu_available():
                            if self.do_grad_scaling:
                                self.scaler.step(self.optimizer)
                    ...
                    if not started_timing and step >= warmup:
                        # Keep this marker for parsing, but allow an alternate format to be enabled.
                        print(f"Start clock at step {step} (post-warmup, unified)!")
                        start_training = time.time()
                        started_timing = True

                    # Learning rate scheduler step (only when not checkpointing with multistream)
                    if not (should_enter_checkpoint_block and self.cfreq > 0):
                ...
                # Printing every step is extremely expensive and can dominate short benchmarks.
                # Enable detailed step timing with: PCCHECK_STEP_LOG_EVERY=N (e.g. 10)
                step_log_every = int(os.environ.get("PCCHECK_STEP_LOG_EVERY", "0") or "0")
                if step_log_every > 0 and (step % step_log_every == 0):
                    print(f"Step {step} took {time.time()-starts}")
                starts = time.time()

            # 关闭 multistream checkpoint
            # 关键：必须先等待所有异步 checkpoint 完成并落盘，否则 EXECUTION TIME 会漏算尾部 IO
            # 且后台线程会持续抢占 CPU/SSD 带宽，导致吞吐统计出现反直觉波动。
    ...
    def _load_best_model(self):
        ...
        if (
            ...
            if self.is_deepspeed_enabled:
                deepspeed_load_checkpoint(
                    self.model_wrapped, self.state.best_model_checkpoint
                )
            else:
    ...
    def _push_from_checkpoint(self, checkpoint_folder):
        ...
        try:
            ...
            push_work = self.repo.push_to_hub(
    ...
    def create_accelerator_and_postprocess(self):
        ...
        if self.is_fsdp_enabled:
            ...
            fsdp_plugin.use_orig_params = self.args.fsdp_config.get(
